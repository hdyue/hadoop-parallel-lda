...
implements Reducer<Text, DocumentWritable, Text, DocumentWritable>{
  ...
  throws IOException {
    while (values.hasNext()) {
      ...
      for (int i = 0; i < doc.getNumWords(); i++) {
        ...
        likelihood += 
          computeSamplingProbability(nzd, word, probs, alpha, beta, doc_length-1);
  ...
  public double computeSamplingProbability(
    ...
    for (int i = 0; i < numTopics; i++) {
      ...
      double pzd = (nzd[i] + alpha) / (doc_length + numTopics * alpha);
      probs[i] = pwz * pzd;
      norm += probs[i];
      likelihood += pwz;
    }
  ...
  public void configure(JobConf conf) {
	...
	private void loadModelParameters(String modelParamFile) throws IOException {
		...
		for (int i = 0; i < numWords; i++) {
			...
			if (delta_nwz[i] == null) {
				delta_nwz[i] = new int[numTopics];
			}
			Arrays.fill(delta_nwz[i], 0);
		}
		fr.close();
